...
public class Thread4_4 extends Thread {

	int[][] vidaAtual;
	int m;

	public Thread4_4(int[][] vidaAtual, int m) {
	...
	public void run() {
		...
		int[][] futuraGeracao = new int[vidaAtual.length][vidaAtual.length];
			...
			// Esse laço passa por cada linha depois da metade
			for (int j = (vidaAtual.length/2); j < vidaAtual.length; j++) {
		...
		JogoDaVidaThreads.preencher4ThreadsParte4(futuraGeracao);
	}
}
